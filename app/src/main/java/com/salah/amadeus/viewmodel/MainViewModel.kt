package com.salah.amadeus.viewmodel

import androidx.lifecycle.*
import androidx.paging.cachedIn
import com.google.gson.Gson
import com.google.gson.reflect.TypeToken
import com.salah.amadeus.repository.DBRepository
import com.salah.amadeus.repository.NetworkRepository
import com.salah.amadeus.models.Data
import com.salah.amadeus.utils.DataHandler
...
class MainViewModel @Inject constructor(
    ...
    private val _state = MutableLiveData<DataHandler<ArrayList<Data>>>()
    val state: LiveData<DataHandler<ArrayList<Data>>> = _state
    ...
    * we are using switchMap to get either all or filtered data based on query string*/
        ...
        viewModelScope.launch {
            withContext(IO) {
                if (dbRepository.isDbEmpty()) {
                    _state.postValue(DataHandler.LOADING())
     ...
     */
    private fun handleResponse(response: Response<ResponseBody>): DataHandler<ArrayList<Data>> {
        if (response.isSuccessful) {
            response.body()?.let { it ->
                val gson = Gson()
                val type = object : TypeToken<ArrayList<Data>>() {}.type
                val body = it.string()
                return DataHandler.SUCCESS(gson.fromJson(body, type))
            }
        }
